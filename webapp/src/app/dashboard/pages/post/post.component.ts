...
export class PostComponent implements OnInit {
    ...
    this.route.params.subscribe(async (params) => {
      if (!params.id) {
        ...
        try {
          ...
          this.hasAgreed = this.post.agreed
            .map((u) => u._id)
            .includes(this.loggedUser._id);
          this.hasDisagreed = this.post.disagreed
            .map((u) => u._id)
            .includes(this.loggedUser._id);
        } catch (exception) {
  ...
  async agree() {
    if (!this.hasAgreed) {
      const disagreedIndex = this.post.disagreed
        .map((u) => u._id)
        .indexOf(this.loggedUser._id);
      if (disagreedIndex > -1) {
  ...
  async disagree() {
    if (!this.hasDisagreed) {
      const agreedIndex = this.post.agreed
        .map((u) => u._id)
        .indexOf(this.loggedUser._id);
      if (agreedIndex > -1) {
